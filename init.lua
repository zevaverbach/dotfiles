...
function SortCSV()
	...
	-- Extract the prefix (any --command), quoted part, and trailing comma
	local prefix, quoted, trailing = line:match('^(%s*%-%-%w+,%s*)"(.-)"(.-)$')

	if not prefix or not quoted then
		print("Line format not recognized")
		return
	end

	-- Split the quoted part into a table
	local items = vim.split(quoted, ",")
	...
	-- Join the items back into a quoted string without extra spaces, preserving the trailing comma
	local sorted_line = prefix .. '"' .. table.concat(items, ",") .. '"' .. trailing

	-- Replace the current line with the sorted line
	vim.api.nvim_set_current_line(sorted_line)
end
-- Create a command to call the function
vim.api.nvim_create_user_command("SortCSV", SortCSV, {})
...
vim.g.goyo_bg = "light" -- or 'light' depending on your preference
