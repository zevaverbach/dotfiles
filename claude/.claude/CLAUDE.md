...
🗣️ Vennari: [summary in 8-16 word summary]
```
...
### Dotfiles Backup (Daily)

At the end of the first conversation of each day (or if explicitly asked), commit and push any changes in `~/repos/dotfiles`:

```bash
cd ~/repos/dotfiles && git add -A && git commit -m "daily backup $(date +%Y-%m-%d)" && git push
```
